...
public class DocumentController {
    ...
    @PostMapping
    public ApiResponse<DocumentResponse> uploadDocument(@AuthenticationPrincipal @Parameter(hidden = true) JobSeeker jobSeeker,
                                                        DocumentUploadRequest request){
        return ApiResponse.ok(documentService.uploadDocument(jobSeeker, request));
    }

    @Operation(summary = "문서 목록 조회")
    @GetMapping
    public ApiResponse<List<DocumentResponse>> getDocuments(@AuthenticationPrincipal @Parameter(hidden = true) JobSeeker jobSeeker){
        return ApiResponse.ok(documentService.getDocuments(jobSeeker));
    }
