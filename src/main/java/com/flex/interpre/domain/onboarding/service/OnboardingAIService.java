...
public class OnboardingAIService {
    ...
    public OnboardingChatResponse chat(User user, OnboardingChatRequest request) {
        ...
        if (isCompleted) {
            ...
            updateJobSeekerInfo(user.getId(), result);
        }
    ...
    private String buildSystemPrompt() {
        return """
            ...
            ### 직무 대분류 (JobFirst):
            DEVELOPMENT, DESIGN, PLANNING, MARKETING, BUSINESS, MANAGEMENT, 
            HR, ACCOUNTING, PRODUCTION, DISTRIBUTION, CUSTOMER_SERVICE, RESEARCH
            
            ### 직무 중분류 (JobSecond):
            FRONTEND, BACKEND, FULLSTACK, MOBILE, INFRA, DATA, AI, 
            UI_UX, GRAPHIC, VIDEO, PRODUCT, SERVICE, DIGITAL, CONTENT,
            SALES, CS, MD, SCM, FINANCE, LEGAL
    ...
    @Transactional
    private void updateJobSeekerInfo(UUID userId, OnboardingResult result) {
        JobSeeker jobSeeker = jobSeekerRepository.findByUserIdWithUser(userId)
        ...
        log.info("구직자 온보딩 완료: userId={}, areas={}, jobFirst={}",
                userId, result.recommendedAreas(), result.recommendedJobFirsts());
    }

    // 채팅 히스토리 조회
    public List<OnboardingSessionCache.ChatMessage> getChatHistory(UUID userId) {
    ...
    public void resetSession(UUID userId) {
        ...
        log.info("온보딩 세션 초기화: userId={}", userId);
    }
}