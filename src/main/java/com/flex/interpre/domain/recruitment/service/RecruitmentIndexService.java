...
public class RecruitmentIndexService {
    ...
    // 공고문 단건 인덱싱
    public void indexRecruitment(Recruitment recruitment) {
        try {
            String textToEmbed = buildFullText(recruitment);
            ...
            throw new IllegalStateException("Failed to index recruitment: " + recruitment.getId(), e);
        }
    }

    // 공고문 데이터 벌크 인덱싱
    public void bulkIndexRecruitments(List<Recruitment> recruitments) {
        try {
    ...
    // KNN 기반 벡터 검색
    public List<Recruitment> searchByVector(List<Double> queryVector, int k) {
        try {
    ...
    private void sendBulk(List<Recruitment> recruitments) {
        try {
